...
void create_binary_tree() {
    ...
    while (!is_empty(&q)) {
        ...
        // Create left child
        printf("Enter value of left child of %d (if not present enter -1): ", p_parent->data);
        scanf("%d", &value);
        if (value != -1) {
...
void inorder_printing(Node* ptr) {
    if (ptr) {
        inorder_printing(ptr->left_child);
        printf("%d ", ptr->data);
        inorder_printing(ptr->right_child);
    }
}

void postorder_printing(Node* ptr) {
    if (ptr) {
        postorder_printing(ptr->left_child);
        postorder_printing(ptr->right_child);
        printf("%d ", ptr->data);
    }
}
