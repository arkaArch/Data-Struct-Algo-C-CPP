...
int main() {
    ...
    */
    ...
    std::weak_ptr<UDT> ptr2;
    {
        ...
        // Now I can check the weak_pointer destroyed or not, and the object which ptr2 pointed
        // is not destructed yet. So the answer must be false(0).
        std::cout << "Is ptr2(object that it points to) freed: " << ptr2.expired() << std::endl;

        // Then we check our updated reference count
        std::cout << "Use count = " << ptr1.use_count() << std::endl;
    }
    // Here the weak_pointer(the object which ptr2 pointed) is destroyed, so it must return true(1).
    std::cout << "Is ptr2(object that it points to) freed: " << ptr2.expired() << std::endl;
