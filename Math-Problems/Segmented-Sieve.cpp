...
void printPrimesSegmentedSieve(int lowerLimit, int upperLimit) {
    ...
    // Lets say we've to find primes between 110 and 130. So we have to find all primes till 11.
    // primes = [2, 3, 5, 7, 11]
    ...
    for (int prime : primes) {
        ...
        // first multiple of 2 is (110/2)*2 = 110, 3 -> (110/3)*3 = 36*3 = 108 < 110 -> 108+3 = 111
        if (firstMultiple < lowerLimit)
            firstMultiple += prime;

        for (long long j = max(firstMultiple, prime * prime); j <= upperLimit; j += prime)
            // Since first multiple of 11 is (110/11)*11 = 110 but we start from 121.(Same as sieve)
